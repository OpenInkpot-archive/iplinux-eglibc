...
$(stamp)configure_%: $(stamp)mkbuilddir_%
	...
	if [ $(curpass) = libc ]; then \
	  $(call logme, -a $(log_build), \
	    mkdir -p $(DEB_BUILDDIR)/localedef && \
		cd $(DEB_BUILDDIR)/localedef && \
	    CPPFLAGS= $(CURDIR)/$(DEB_SRCDIR)/localedef/configure \
	       --with-glibc=$(CURDIR)/$(DEB_SRCDIR)) \
	fi
...
$(stamp)build_%: $(stamp)configure_%
	...
	if [ $(curpass) = libc ]; then \
	  $(call logme, -a $(log_build), $(MAKE) -C $(DEB_BUILDDIR)/localedef $(NJOBS)); \
	  $(call logme, -a $(log_build), \
	    mkdir -p $(CURDIR)/$(DEB_BUILDDIR)/localedef/locales; \
	    for i in $(shell cat $(CURDIR)/debian/locales-list); do \
	      I18NPATH=$(CURDIR)/$(DEB_SRCDIR)/localedata $(DEB_BUILDDIR)/localedef/localedef \
	          -i \$$i -f UTF-8 $(CURDIR)/$(DEB_BUILDDIR)/localedef/locales/\$$i.UTF-8; \
		done); \
	fi
	touch $@
...
# Install headers no matter what
	...
	if [ $(curpass) = libc ]; then \
	  ...
	  (cd $(DEB_BUILDDIR)/localedef/locales \
